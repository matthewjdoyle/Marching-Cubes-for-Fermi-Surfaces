...
Generates high-resolution Density of States (DOS) plots and 3D rotating view
...
LATTICES = {
    'sc': {
        ...
        'yticks': [0.000, 0.025, 0.050, 0.075, 0.100, 0.125, 0.150, 0.175, 0.200]
    },
    'sc_nnn': {
...
def process_lattice(lat_id, lat_info):
    ...
    # ── 4. Generate 3D Fermi Surface Sweep Video ──
    FPS = 30
    DURATION = 8  # seconds
    ...
    def update_frame(frame_idx):
        ...
        # Camera viewpoint rotating dynamically
        cam_elev = 22 + 10 * np.sin(2 * np.pi * frame_idx / TOTAL_FRAMES)
        cam_azim = 360 * frame_idx / TOTAL_FRAMES
    ...
    video_out_path = os.path.join(OUTPUT_DIR, f"fermi_surface_evolution_{lat_id}_rotating.mp4")
    print(f"Generating Rotating Fermi Surface Evolution Video ({TOTAL_FRAMES} frames)...")
